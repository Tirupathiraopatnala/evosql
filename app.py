...
def render_execution_panel(gen, total_gens, exec_agent_ids, exec_agent_id, execution_cards):
    with live_placeholder.container():
        ...
        # Rewrite summary badges
        if st.session_state.cur_agent_ids:
            cols = st.columns(min(len(st.session_state.cur_agent_ids), 6))
            for i, aid in enumerate(st.session_state.cur_agent_ids):
                short = aid[:8]
                col   = cols[i % len(cols)]
                card  = next((c for c in st.session_state.cur_agent_cards
                              if c.get("agent_id") == aid), None)
                if card and card.get("ok"):
                    col.markdown(f"✅ `{short}`")
                elif card and card.get("warn"):
                    col.markdown(f"⚠️ `{short}`")
